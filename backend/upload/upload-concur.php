<?php
	...
	function subir_imagen($file){
		...
		if($file && $file["name"]!=""){
			...
			$direccion_web = "http://crd-label.com.ar/backend/upload/img/concursos/";
			$nombre_carpeta = "img/concursos/";
	...
	if(isset($_POST["submit"])){
		...
		$titulo_en = mysqli_real_escape_string($conexion,$_POST["titulo_en"]);

		$texto = mysqli_real_escape_string($conexion,$_POST["texto"]);

		$texto_en = mysqli_real_escape_string($conexion,$_POST["texto_en"]);

		$descripcion = mysqli_real_escape_string($conexion,$_POST["descripcion"]);

		$descripcion_en = mysqli_real_escape_string($conexion,$_POST["descripcion_en"]);
		// LEVANTO EL NOMBRE DEL ARCHIVO USANDO LA FUNCION subir_imagen DECLARADA ARRIBA
		$imagen = subir_imagen($_FILES["imagen_a_subir"]);
		//$imagen = subir_imagen($_FILES["archivo_a_subir"]);
		$archivo = subir_archivo($_FILES["archivo_a_subir"]);

		if($imagen=="error"){
		//if($imagen=="error"){
			echo "Error. Hubo un error con la imagen, por favor revisar</br>";
			echo '<a href="formulario-concurso.php">Volver atras</a>';
			...
			$sql = "insert into concursos (titulo, titulo_en, imagen, archivos, texto, texto_en, descripcion, descripcion_en) values ('$titulo','$titulo_en','$imagen','$archivos','$texto','$texto_en','$descripcion','$descripcion_en')";
